...
class Admin {
	...
	public static function register_post_type() {
		$labels = [
			...
			'all_items'          => __( 'All Forms', self::POST_TYPE ),
	...
	public function add_meta_boxes() {
		global $post_type;
		if( $post_type == self::POST_TYPE ) {
			add_meta_box( 'orderform_field', __( 'Embed Form', self::POST_TYPE ), [ $this, 'add_metabox' ], self::POST_TYPE, 'normal' );
		}
	}
	...
	public function add_metabox( $post ) {
		...
		<div>
			...
			<input style='width:100%'  type="text" value="<?php echo $value?>" name="orderforms_id" size="70" placeholder="Please enter url like this format :  {DOMAINWITHSSL/}.orderforms.com/order/form/{FORMID}" pattern="https://+[A-Za-z0-9]+.orderforms.com/order/form/.+[A-Za-z0-9]"/ required>	
		</div><span style='font-size:12px;color:red'><strong>Note:</strong> Please enter the full form Url here</span><?php 
	}

	public function admin_settings_nav() {
		add_submenu_page( 'edit.php?post_type=' . self::POST_TYPE, 'OrderForms Settings', __( 'Settings' ), 'manage_options', 'settings', [ $this, 'settings_page' ] );
	}
	...
	public function settings_page() {
		...
		<div class="wrap">
            ...
            <div>
				...
				<?php echo get_site_url(); ?>/<input type="text" name="orderforms[slug]" value="<?php echo @$options[ 'slug' ]?>">/formname
			</div>
			<span style='font-size:12px;color:red'><strong>Note:</strong> After changing the slug please update "Permalink Settings" by clicking on 'Save Changes' button (Reference URL : {DomainName}/wp-admin/options-permalink.php)</span>	
